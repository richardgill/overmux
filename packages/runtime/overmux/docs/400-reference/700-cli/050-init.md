...
The command validates the toolchain before writing files. When Mise is installed, it must be [activated in the shell](https://mise.jdx.dev/getting-started.html#activate-mise); the generated `mise.toml` pins Node 22, pnpm 10, and the latest published Overmux version. Without Mise, pnpm 10.5.0 or newer must already be available and `mise.toml` is omitted.
...
The generated `pnpm-workspace.yaml` allows only `node-pty` to run dependency build scripts, including when you later add `@overmux/tmux`. This approval is project-local; global pnpm settings are unchanged. On Linux, building `node-pty` still requires `node-gyp`, Python, and a native compiler toolchain, which `overmux init` does not install.
